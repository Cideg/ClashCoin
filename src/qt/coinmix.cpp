...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::CLH)
    {
    }
...
void Coinmix::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("CLH")
    //updateDisplayUnit();
}
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeCLHAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeCLHAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeCLHAmount = strAnonymizeCLHAmount.remove(strAnonymizeCLHAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCLHAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeCLHAmount * COIN) nMaxToAnonymize = nAnonymizeCLHAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeCLHAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeCLHAmount));
        strAnonymizeCLHAmount = strAnonymizeCLHAmount.remove(strAnonymizeCLHAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCLHAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
