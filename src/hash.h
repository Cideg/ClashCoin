...
inline uint256 *Hashing(uint256 data)
{
	int i = 0;
	int mask = 16;
	...
	if (d == 0) data = data + data;
	sph_keccak256(&ctx_keccak, static_cast<const void*>(&data), 64);
	sph_keccak256_close(&ctx_keccak, static_cast<void*>(&hash[i]));

	while (i < 16)
	{
		i++;
		sph_keccak256_init(&ctx_keccak);
		if (d == i - 1) hash[i - 1] = hash[i - 1] + hash[i - 1];
		sph_keccak256(&ctx_keccak, static_cast<const void*>(&hash[i - 1]), 64);
		sph_keccak256_close(&ctx_keccak, static_cast<void*>(&hash[i]));
	}
...
template <typename T1>
inline uint256 ClashHash(const T1 pbegin, const T1 pend)

{
	uint256 init;
	uint256 old_hash[4096] = { 0 };
	uint256 new_hash[4096] = { 0 };
	uint256 reps, r;
	uint256 *h;
	int oh_len;
	sph_keccak256_context ctx_keccak;
	...
	sph_keccak256_close(&ctx_keccak, static_cast<void*>(&init));
	...
	while (r < reps)
	{
		std::copy(std::begin(new_hash), std::end(new_hash), std::begin(old_hash));
		memset(new_hash, 0, sizeof(new_hash));
		oh_len = sizeof(old_hash) / sizeof(uint256);

		while (oh_len > 0)
		{
			oh_len--;
			h = Hashing(old_hash[oh_len]);
			for (int i = 0; i < 16; i++) {
				new_hash[oh_len] = h[i];
			}
		}
	...
	{
		oh_len--;
		new_hash[0] = new_hash[0] + old_hash[oh_len];
	}

	sph_keccak256_init(&ctx_keccak);
	sph_keccak256(&ctx_keccak, static_cast<const void*>(&new_hash[0]), (pend - pbegin) * sizeof(pbegin[0]));
	sph_keccak256_close(&ctx_keccak, static_cast<void*>(&init));
	return init;
}
