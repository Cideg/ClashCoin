...
*Setup instructions for a gitian build of CLH using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the CLH
...
Clone the git repositories for clash and gitian and then checkout the clash version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the clash repository
...
To build CLH (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the clash repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/clash/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/jembem/clash
...
./bin/gbuild --commit clash=${COMMIT} --url clash=${URL} ../clash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit clash=${COMMIT} --url clash=${URL} ../clash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit clash=${COMMIT} --url clash=${URL} ../clash/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/clash-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/clash-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/clash-build.assert
```
...
[clash/gitian.sigs](https://github.com/clash-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
